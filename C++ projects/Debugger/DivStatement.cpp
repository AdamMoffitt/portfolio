...
DivStatement::DivStatement(std::string variableName, string p) 
...
// The DivStatement version of execute() makes two changes to the
...
void DivStatement::execute(ProgramState * state, ostream &outf)
{
	...
	if(!myMap[m_variableName]){
		...
		if(state->isNumber(p)){
			...
			if(value == 0){
				outf << "Error: Cannot divide by Zero\n";
				state->setProgramCounter(-100);
				return;
			}
			else{
