...
const IEX_BASE_URL = 'https://api.iextrading.com/1.0/'
...
function aggregateUserPortfolio(portfolio){
  ...
  // console.log(portfolio)
	...
	// '&apikey='+ALPHA_TOKEN
	// console.log(api)
	const api = IEX_BASE_URL+'stock/market/batch?symbols='+stocks.join(',')+'&types=quote'
	console.log(api)
	// return
	// return new Promise((resolve, reject) => {
...
exports.teamMissionControl = functions.https.onRequest((request, response) => {
	...
	return Promise.all([getUserPromise, teamsPromise, leaguesPromise]).then(results => {
		...
		console.log("portfolio here: ", tempPortfolio);
...
exports.giftStock = functions.https.onRequest((request, response) => {
	...
	const api = IEX_BASE_URL+'stock/market/batch?symbols='+ticker+'&types=quote'
	console.log(api);
